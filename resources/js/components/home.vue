      ...
      <div class="col-lg-12 mb-4">
        ...
        <div class="card">
          <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-primary">Out-off Stock</h6>
          </div>
            ...
            <table class="table align-items-center table-flush">
              ...
              <tbody>
                <tr v-for="product in products" :key="product.id">           
                  ...
                  <td><img id="img" :src="'/' + product.image" alt="img" /></td>
                  <td>{{ product.buying_price }}</td>
                  <td v-if="product.product_quantity >= 1"><span class="badge badge-success">
                     Available</span>
                  </td>
...
export default {
   ...
   methods: {
      TodaySell() {
         axios
            .get('api/today/sell')
            .then((response) => { this.todaySell = response.data })
            .catch()
      },
      TodayIncome() {
         axios
            .get('api/today/income')
            .then((response) => { this.todayIncome = response.data })
            .catch()
      },
      TodayDue() {
         axios
            .get('api/today/due')
            .then((response) => { this.todayDue = response.data })
            .catch()
      },
      TodayExpense() {
         axios
            .get('api/today/expense')
            .then((response) => { this.todayExpense = response.data })
            .catch()
      },
      Products() {
         axios
